...
class OpenCartExtensionInstallerPlugin implements PluginInterface
{
    ...
    {
    	$this->io->write("opencart-extension-installer <info>activate method</info>");
        $installer = new OpenCartExtensionInstaller($io, $composer, $this->packageType);
        $composer->getInstallationManager()->addInstaller($installer);
    }

	public function deactivate(Composer $composer, IOInterface $io) {
		$this->io->write("opencart-extension-installer <info>deactivate method</info>");
		$installer = new OpenCartExtensionInstaller($io, $composer, $this->packageType);
		$composer->getInstallationManager()->removeInstaller($installer);
	}
	public function uninstall(Composer $composer, IOInterface $io) {
		$this->io->write("opencart-extension-installer <info>uninstall method</info>");
		$installer = new OpenCartExtensionInstaller($io, $composer, $this->packageType);
		$composer->getInstallationManager()->removeInstaller($installer);
	}
}